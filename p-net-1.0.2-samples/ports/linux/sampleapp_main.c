...
/* OK3576-C RT Linux configuration */
#define APP_DEFAULT_STATION_NAME       "rt-labs-dev"
#define APP_DEFAULT_FILE_DIRECTORY     "/tmp/pnet"
...
app_args_t parse_commandline_arguments (int argc, char * argv[])
{
   ...
   while ((option = getopt (argc, argv, "hvgfri:s:b:d:p:m:")) != -1)
   {
      ...
      case 'i':
         ...
         break;
      case 's':
         if ((strlen (optarg) + 1) > sizeof (output_arguments.station_name))
         {
            printf ("Error: The argument to -s is too long.\n");
            exit (EXIT_FAILURE);
         }
...
int main (int argc, char * argv[])
{
   ...
   APP_LOG_INFO ("Interval (1/32 ms):   %u\n", pnet_cfg.min_device_interval);
   APP_LOG_INFO ("Tick rate (us):       %u\n", pnet_cfg.tick_us);
